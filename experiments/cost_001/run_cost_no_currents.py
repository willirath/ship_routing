#!/usr/bin/env python
"""No-currents ablation runs for cost_001.

Same as run_cost.py but optimizes without current forcing.
Routes are optimized with waves+wind only (currents_path=None).

Usage:
    # Local smoke test (2 journeys, 1 replica)
    python run_cost_no_currents.py --submission-id 1 --execution local-small --quick

    # Full production on SLURM
    python run_cost_no_currents.py --submission-id 1 --execution nesh-prod-40

    # Second submission (independent seeds)
    python run_cost_no_currents.py --submission-id 2 --execution nesh-prod-40

    # Dry run
    python run_cost_no_currents.py --submission-id 1 --execution local-small --dry-run
"""
...
def main(
    ...
    """Run no-currents ablation experiments."""
    ...
    configs = make_production_configs(
        ...
        forcing=FORCING_NO_CURRENTS,
    )

    logger.info(
        f"Generated {len(configs)} no-currents configs "
    ...
    if output:
        ...
        output_path = (
            Path("results")
            / f"cost_no_currents_sub{submission_id:02d}_{timestamp}.msgpack"
        )

    run_dir = (
        Path("runinfo") / f"cost_no_currents_sub{submission_id:02d}_{timestamp}"
    ).resolve()
